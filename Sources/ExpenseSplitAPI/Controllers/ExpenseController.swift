...
struct ExpenseController: RouteCollection {
  ...
  func create(req: Request) async throws -> CreateExpenseResponse {
    ...
    let isParticipant =
      try await ActivityParticipant.query(on: req.db)
    ...
    if let payerId = createRequest.payerId {
      guard (try await User.find(payerId, on: req.db)) != nil else {
        throw LocalizedAbortError(
          status: .notFound,
    ...
    for participantId in createRequest.participantsIds {
      guard (try await User.find(participantId, on: req.db)) != nil else {
        throw LocalizedAbortError(
          status: .notFound,
  ...
  func update(req: Request) async throws -> CreateExpenseResponse {
    ...
    if let newPayerId = updateRequest.payerId {
      guard (try await User.find(newPayerId, on: req.db)) != nil else {
        throw LocalizedAbortError(
          status: .notFound,
          key: .expensePayerNotFound,
          arguments: [:],
  ...
  func markPayment(req: Request) async throws -> MarkPaymentResponse {
    ...
    guard
      let expenseParticipant = try await ExpenseParticipant.query(on: req.db)
        .filter(\.$expense.$id == expenseId)
        .filter(\.$user.$id == user.id!)
        .first()
    else {
  ...
  // MARK: - Toggle Participant Payment Status
  func toggleParticipantPayment(req: Request) async throws -> ToggleParticipantPaymentResponse {
    let user = try req.auth.require(User.self)
    ...
    // Verify authenticated user is participant of the activity
    let isParticipant =
      try await ActivityParticipant.query(on: req.db)
      .filter(\.$activity.$id == expense.$activity.id)
      .filter(\.$user.$id == user.id!)
    ...
    // Verify the participant exists in the expense
    guard
      let expenseParticipant = try await ExpenseParticipant.query(on: req.db)
        .filter(\.$expense.$id == expenseId)
        .filter(\.$user.$id == participantId)
    ...
    // Get current payments for this participant
    let existingPayments = try await ExpensePayment.query(on: req.db)
      .filter(\.$expense.$id == expenseId)
      .filter(\.$debtor.$id == participantId)
      .all()

    let totalPaid = existingPayments.reduce(0) { $0 + $1.amountPaidInCents }
    let isFullyPaid = totalPaid >= expenseParticipant.amountOwedInCents

    // Toggle: if paid, set to pending (clear payments); if pending/partial, set to paid (pay full amount)
    if isFullyPaid {
      // Delete all payments to set to pending
      try await ExpensePayment.query(on: req.db)
        .filter(\.$expense.$id == expenseId)
        .filter(\.$debtor.$id == participantId)
        .delete()
    } else {
      // Delete existing payments and create a new one for the full amount
      try await ExpensePayment.query(on: req.db)
        .filter(\.$expense.$id == expenseId)
        .filter(\.$debtor.$id == participantId)
        .delete()
    ...
    let finalAmountPaid = isFullyPaid ? 0 : expenseParticipant.amountOwedInCents
    let remainingDebt = expenseParticipant.amountOwedInCents - finalAmountPaid
    let paymentStatus = isFullyPaid ? "pending" : "paid"

    return ToggleParticipantPaymentResponse(
      expenseId: expenseId,
      participantId: participantId,
      participantName: expenseParticipant.user.name,
      participantEmail: expenseParticipant.user.email,
      amountOwedInCents: expenseParticipant.amountOwedInCents,
