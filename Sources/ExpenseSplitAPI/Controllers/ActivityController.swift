...
struct ActivityController: RouteCollection {
  ...
  func detail(req: Request) async throws -> ActivityDetailResponse {
    ...
    guard
      let activity = try await Activity.query(on: req.db)
    ...
    for expense in activity.expenses {
      ...
      for ep in expenseParticipants {
        // Get all payments made by this participant for this expense
        let payments = try await ExpensePayment.query(on: req.db)
          .filter(\.$expense.$id == expense.id!)
          .filter(\.$debtor.$id == ep.$user.id)
          .all()
